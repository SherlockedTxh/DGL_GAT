...
if __name__ == '__main__':
    ...
    in_dir = Final_value.final_path
    out_dir = Final_value.final_path + '/graph'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    files = [f for f in os.listdir(in_dir) if os.path.isfile(os.path.join(in_dir, f))]

    i = 0
    for filename in os.listdir(Final_value.windows_dir_pre):
        infile = open(in_dir + '/' + filename + '.txt')
        datalist = list(infile)
        read_data(i, datalist)
        i += 1
        np.save(out_dir + '/' + filename + '_feats', np.asarray(features))
        np.save(out_dir + '/' + filename + '_graph_id', np.asarray(graph_id))
        np.save(out_dir + '/' + filename + '_labels', np.asarray(labels))
