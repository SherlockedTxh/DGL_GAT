...
if __name__ == '__main__':
    in_dir = Final_value.final_path + '/graph'
    out_dir = Final_value.final_path + '/graph'

    for filename in os.listdir(Final_value.windows_dir_pre):
        test = np.load(in_dir + '/' + filename + '_feats.npy')
        test_graph_id = np.load(in_dir + '/' + filename + '_graph_id.npy')
        # print(test_graph_id)
        ...
        fileObject = open(in_dir + '/' + filename + '_graph.json', 'w')
    ...
    # npy_to_graph(test, test_graph_id)
    # test_graph_dict = {"directed": True, "multigraph": False, "graph": {}, "nodes": nodes, "links": links}
    # test_graph_json = json.dumps(valid_graph_dict)
