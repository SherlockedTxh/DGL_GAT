...
class AcidDataset(object):
    ...
    def _load(self):
        ...
        """
        name = 'ppi'
        dir = windows_dir_pre = "/mnt/md1/a503tongxueheng/3DCNN_data_process/data/output/graph/"
        ...
        if self.mode == 'train':
            ...
            self.features = np.load(dir+'train_feats.npy'.format(dir))
        ...
        if self.mode == 'valid':
            ...
            self.features = np.load(dir+'valid_feats.npy'.format(dir))
            self.graph = DGLGraph(nx.DiGraph(json_graph.node_link_graph(g_data)))
        ...
        if self.mode == 'test':
            ...
            self.features = np.load(dir+'test_feats.npy'.format(dir))
            self.graph = DGLGraph(nx.DiGraph(json_graph.node_link_graph(g_data)))
    ...
    def _preprocess(self):
        if self.mode == 'train':
            ...
            self.train_graphs = []
            self.train_labels = []
            for train_graph_id in range(0, 3986):
        ...
        if self.mode == 'valid':
            ...
            self.valid_graphs = []
            self.valid_labels = []
            for valid_graph_id in range(3987, 4484):
        ...
        if self.mode == 'test':
            ...
            self.test_graphs = []
            self.test_labels = []
            for test_graph_id in range(4485, 4983):
...
class LegacyPPIDataset(AcidDataset):
